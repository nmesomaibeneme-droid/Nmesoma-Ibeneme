      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
          {WHY_MH_JESSE.map((item, idx) => (
            <div
              key={idx}
              className="bg-white border border-slate-200 rounded-sm p-6 hover:border-mh-blue/60 transition-colors shadow-xs"
            >
              <div className="w-8 h-8 rounded-sm bg-mh-blue flex items-center justify-center text-white font-bold text-xs mb-4 shadow-xs">
                0{idx + 1}
              </div>
              <h3 className="text-base font-bold text-mh-dark mb-2">{item.title}</h3>
              <p className="text-xs text-slate-600 leading-relaxed">{item.desc}</p>
            </div>
          ))}

          {/* 100% Nigerian Content Highlight Box */}
          <div className="bg-mh-dark text-white border border-slate-800 rounded-sm p-6 flex flex-col justify-between shadow-xs">
            <div>
              <div className="flex items-center gap-2 text-mh-blue font-bold text-xs uppercase tracking-wider mb-2">
                <ShieldCheck className="w-4 h-4 text-mh-blue" /> 100% Nigerian Local Content
              </div>
        ...
        <div className="bg-mh-dark text-white rounded-sm p-8 lg:p-10 border border-slate-800 shadow-sm">
          <div className="text-center max-w-2xl mx-auto mb-10">
            <span className="text-xs font-bold text-mh-blue uppercase tracking-widest">Integrated Delivery</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {PROJECT_FRAMEWORK.map((stage) => (
              <div
                key={stage.step}
                className="bg-slate-800/90 p-5 rounded-sm border border-slate-700 flex flex-col justify-between hover:border-mh-blue/50 transition-colors"
              >
                <div>
                  <div className="w-7 h-7 rounded-sm bg-mh-blue text-white font-bold text-xs flex items-center justify-center mb-3">
                    {stage.step}
                  </div>
                  ...
                  <ul className="space-y-1.5 text-xs text-slate-300">
                    {stage.points.map((pt, pIdx) => (
                      <li key={pIdx} className="flex items-start gap-1.5">
                        <span className="text-mh-blue font-bold">•</span>
                        <span>{pt}</span>
                      </li>
                    ))}
                  </ul>
