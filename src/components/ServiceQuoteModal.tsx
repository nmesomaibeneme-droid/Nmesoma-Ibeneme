    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs overflow-y-auto">
      <div className="bg-white rounded-sm max-w-2xl w-full border border-slate-200 shadow-xl overflow-hidden my-8 relative">
        ...
        <div className="bg-slate-900 text-white p-6 border-b border-slate-800 flex items-center justify-between">
          <div>
            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest block">Direct Inquiry & RFQ</span>
        ...
        {submitted ? (
          <div className="p-8 text-center space-y-6">
            <div className="w-12 h-12 bg-slate-900 text-white rounded-sm flex items-center justify-center mx-auto">
              <CheckCircle2 className="w-7 h-7" />
            </div>

            <div>
              <h4 className="text-xl font-bold text-slate-900">Proposal Request Generated</h4>
              <p className="text-xs text-slate-600 mt-2 max-w-md mx-auto leading-relaxed">
                Thank you, <strong className="text-slate-900">{formData.fullName}</strong>. Your technical request for <strong className="text-slate-900">{formData.companyName || 'your organization'}</strong> has been generated for MH Jesse Nigeria Limited.
              </p>
            </div>

            {/* Formatted Request Box */}
            <div className="bg-slate-900 text-slate-200 p-4 rounded-sm text-left text-xs font-mono whitespace-pre-wrap max-h-48 overflow-y-auto border border-slate-800">
              {formatSummaryText()}
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
              <button
                ...
                className="w-full sm:w-auto bg-slate-100 hover:bg-slate-200 text-slate-900 font-bold text-xs uppercase tracking-wider px-5 py-2.5 rounded-sm flex items-center justify-center gap-2 border border-slate-300 transition-colors"
              >
              ...
              <a
                ...
                className="w-full sm:w-auto bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs uppercase tracking-wider px-5 py-2.5 rounded-sm flex items-center justify-center gap-2 transition-colors"
              >
            ...
            <button
              ...
              className="text-xs text-slate-500 hover:text-slate-800 font-bold uppercase tracking-wider underline block mx-auto pt-2"
            >
              Close Window
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full text-xs p-2.5 rounded-sm border border-slate-300 text-slate-900 focus:border-slate-900 focus:outline-none"
                />
              </div>
            ...
            <div className="pt-2 flex items-center justify-end gap-3 border-t border-slate-100">
              ...
              <button
                type="submit"
                className="bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs uppercase tracking-wider px-6 py-2.5 rounded-sm transition-colors flex items-center gap-2"
              >
